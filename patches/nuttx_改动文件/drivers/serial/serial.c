...
/* ⚠ 2026-09-15：控制台（这块板子的控制台就是芯片内置 USB 串口）在主机不把
 * 数据读走时（只插 USB 供电、或者串口工具关了但端口还开着），TX FIFO 会一直
 * 是满的。NuttX 原来在这里是无限期等待 / 死循环，结果谁 printf 谁被挂死
 * （现象：UI 冻住、断网、心跳停、wifi 扫描永远出不来；实测把串口读起来就
 * 立刻恢复）。控制台改成限时等待，超过这个毫秒数就把积压输出丢掉。 */
      ...
      else if (oktoblock)
        {
           ...
           */
          ...
          else
            {
              ...
              if (dev->isconsole)
                {
                  /* 控制台在主机不读走数据时这个信号量永远等不到，改成限时
                   * 等待：超时就当"发完了"，把积压的待发数据整段丢掉，放调用
                   * 者继续跑。有人在看串口时行为完全不变。 */
  ...
  while (len > 0)
    {
      ...
      while (!uart_txready(dev))
        {
          /* 控制台主机不读走数据时 txready 会一直是 false，原来是死循环，
           * 而且调用点通常关着中断 —— 命中就是整个系统锁死。给控制台上限，
           * 等不到就丢掉剩下的输出。 */

          if (dev->isconsole &&
              clock_systime_ticks() - start >= MSEC2TICK(CONSOLE_TXWAIT_MSEC))
            {
              return;
            }
        }
          ...
          while (ret >= 0 && dev->xmit.head != dev->xmit.tail)
            {
              ...
              if (dev->isconsole)
                {
                  /* 同上：控制台卡住时 tcdrain 也会一直等下去，限时后丢弃 */
